<template>
  <v-row no-gutters class="flex-column">
    <div>
      <AppBanner :bannerImage="bannerImage" title="Everything" />
    </div>
    <v-btn v-if="waitingData" x-large text plain loading></v-btn>
    <div v-else class="pt-5" id="news_sections">
      <v-container>
        <h3 class="headline font-weight-bold my-9">Top General Headlines</h3>
...
export default {
  ...
  methods: {
    async fetchGlobalHeadlines() {
      ...
      let response = await this.$store.dispatch('TopHeadlines', 'general')
